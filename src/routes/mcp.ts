...
export default async function mcpRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/mcp-web', {
    preHandler: authWebMiddleware, // Use web auth middleware
  }, async (request, reply) => {
    const mcpRequest = request.body as any;
    const user = request.user!; // Set by auth middleware
    ...
    try {
        ...
        if (toolName) {
          ...
          if (GLOBAL_BLACKLIST.includes(actionPart)) {
            logger.warn({
              ...
              toolName,
              actionPart,
            }, 'BLOCKED: Globally blacklisted destructive operation attempted (web)');
          ...
          if (!isToolAllowed(toolName, userPlan)) {
            ...
            logger.warn({
              ...
              toolName,
              userPlan,
            }, 'BLOCKED: Tool not allowed for user plan (web)');
      ...
      logger.debug({
        ...
        tokenLength: accessToken.length,
        requestBody: JSON.stringify(mcpRequest).substring(0, 500)
      }, 'Forwarding to Amazon MCP endpoint (web)');
      ...
      logger.info({
        ...
        ok: response.ok,
        contentType: contentType
      }, 'Amazon MCP response status (web)');
      ...
      logger.debug({
        responseLength: responseText.length,
        responsePreview: responseText.substring(0, 300)
      }, 'Raw response from Amazon (web)');

      let result;
      try {
        result = JSON.parse(responseText);
      } catch (e) {
        logger.error({ error: String(e), responseText: responseText.substring(0, 500) }, 'Failed to parse JSON response (web)');
        throw new Error('Invalid JSON response from Amazon MCP');
      }
      ...
      logger.debug({
        ...
        isNull: result === null,
        resultSample: JSON.stringify(result).substring(0, 200)
      }, 'Amazon MCP response received (web)');
      ...
      if (result === null) {
        logger.warn({
          ...
          contentType: contentType,
          method: mcpRequest.method
        }, 'Amazon MCP returned null response (web)');
      ...
      if (mcpRequest.method === 'tools/list' && result.result && result.result.tools) {
        ...
        const filteredAmazonTools = result.result.tools.filter((tool: any) => {
          ...
          if (isAccountListingTool) {
            logger.info({ toolName: tool.name }, 'Filtered out Amazon account listing tool (web)');
          }
        ...
        logger.info(
          ...
          'Injected Geenie custom tools and filtered Amazon account tools (web)'
        );
      }

      logger.info({ method: mcpRequest?.method }, 'MCP request completed (web)');

      return result;
    } catch (error: any) {
      logger.error({ error: error.message }, 'MCP proxy error (web)');
